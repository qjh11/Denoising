...
switch test
    case 'real'
load('seismic_noisy.mat');
disp('Field Seismic Data')
exampledata = single(exampledata);
data.train_noisy = [exampledata(1,:)];

 % data.test_noisy = test_data_noisy;
data.noise = data.train_noisy(1,:)'; 
% 生成时间轴（如果需要）
data.dt = 0.01;  
data.t = (1:size(data.train_noisy, 2)) * data.dt;

% load('Example_Data.mat');
% dataex = single(dataex);
% data.train_noisy = dataex(1,:);
% data.noise = data.train_noisy(1,:)';
% % 生成时间轴
% data.dt = 0.01;  % 根据采样间隔设置
% data.t = (1:size(data.train_noisy, 2)) * data.dt;
...
% data.dt = data.hdr.times.delta;        % deltass获取采样间隔
...
% 将列信号展平为一个长向量
Xnoisy_flattened = Xnoisy(:); % 原始信号展平
denoised_flattened = denoised_norm(:); % 去噪信号展平
% 计算整个信号集的RMSE
rmse_overall = sqrt(mean((Xnoisy_flattened - denoised_flattened).^2));
% 显示整体RMSE
disp(['整体RMSE: ', num2str(rmse_overall)]);
...
SNR = calculate_snr(Xnoisy, denoised_norm);
fprintf('去噪后的信噪比（SNR）: %f dB\n', SNR);
...
snr_value = calculate_snr(data.x, denoised_norm)
...
% 归一化函数，将数据通过 Z-score 标准化，并将结果缩放到 [-1, 1] 范围
...
function rmse = calculateRMSEWithWavelet(y)
    % y: 输入信号
    % 小波变换
    [C, L] = wavedec(y, 4, 'db1');  % 使用Daubechies小波进行4层变换

    % 重构信号
    y_reconstructed = waverec(C, L, 'db1');

    % 计算均方根误差
    rmse = sqrt(mean((y - y_reconstructed).^2));
end



% function snr_value = calculate_snr(x, denoised_norm)
%     % 计算噪声部分
%     denoised_norm = denoised_norm';
%     x = reshape(x(1:1000), [1000, 1]);
%     noise = x - denoised_norm;
% 
%     % 计算 SNR
%     snr_value = 10 * log10(sum(x .^ 2) / sum(noise .^ 2));
% end

function snr_value = calculate_snr(Xnoisy_flattened, denoised__flattened)
    % 计算噪声部分
    denoised__flattened = denoised__flattened';
    noise = Xnoisy_flattened - denoised__flattened;
    
    % 计算 SNR
    snr_value = 10 * log10(sum(denoised__flattened .^ 2) / sum(noise .^ 2));
end
